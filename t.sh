...
# 端口转发工具管理脚本
# 作者: Joey
# 版本: 2.1 (修复了在新版Linux上的依赖安装问题 - PEP 668)
...
# (此处省略了 check_root, show_header, show_menu 等未改动的函数, 完整版在最后)
...
install_dependencies() {
    ...
    if [[ "$OS_TYPE" == "Linux" ]] && command -v apt-get &> /dev/null; then
        echo -e "${CYAN}检测到 Debian/Ubuntu 系统，尝试使用 apt 安装 (需要sudo权限)...${NC}"
        sudo apt-get update
        sudo apt-get install -y python3-flask python3-werkzeug
        if [ $? -eq 0 ]; then
            echo -e "${GREEN}✓ 依赖 (python3-flask, python3-werkzeug) 安装成功${NC}"
        else
            echo -e "${RED}✗ apt 安装失败。请检查错误信息。${NC}"
            return 1
        fi
    else
        echo -e "${YELLOW}正在使用 pip 安装Python依赖 (flask, werkzeug)...${NC}"
        # 在非Debian系Linux或macOS上，继续使用pip
        python3 -m pip install --user flask werkzeug
        if [ $? -eq 0 ]; then
            echo -e "${GREEN}✓ 依赖安装成功${NC}"
        else
            echo -e "${RED}✗ pip 安装失败。${NC}"
            echo -e "${YELLOW}如果是在新版Linux上，您可能遇到了PEP 668错误。${NC}"
            echo -e "${YELLOW}请尝试手动运行 'sudo apt install python3-flask python3-werkzeug'。${NC}"
            return 1
        fi
    fi
...
show_header() {
    clear
    echo -e "${CYAN}========================================${NC}"
    echo -e "${CYAN}     专业端口转发工具管理脚本 v2.1${NC}"
    echo -e "${CYAN}========================================${NC}"
    echo
}
...
# (此处省略 view_config, backup_data, restore_data, clean_data, test_*, download_latest 等未改动函数以保持简洁)
# ...
# 它们与上一版完全相同
